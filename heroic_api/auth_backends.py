...
from heroic_api import hopskotch
from heroic_api.models import Profile

logger = logging.getLogger(__name__)


class HeroicTokenAuthentication(TokenAuthentication):
...
class HopskotchOIDCAuthenticationBackend(oidc_auth.OIDCAuthenticationBackend):
    ...
    def create_user(self, claims):
        """Create a HEROIC Django User with
        ...
        HEROIC requires that a SCiMMA Auth User exists. So, before creating the Heroic User,
    ...
    def update_user(self, user, claims):
        ...
        Base class extensions:
          ...
          * check that SCiMMA Auth User exists and create if necessary.
        ...
        user.is_superuser = is_member_of(claims, '/SCiMMA Developers')
        user.save()
...
def hopskotch_logout(request):
    """Do the actions required when the user logs out of HEROIC (and thus hopskotch).
    ...
    url_args = {'post_logout_redirect_uri': settings.HEROIC_FRONT_END_BASE_URL, 'client_id': settings.OIDC_RP_CLIENT_ID, 'id_token_hint': id_token}
